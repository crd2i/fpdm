...
if (!call_user_func_array('class_exists', $__tmp)) {
	...
	define('FPDM_CACHE', "uploads/tmp/");//dirname(__FILE__).'/export/cache/'); //cache directory for fdf temporary files needed by pdftk.
	define('FPDM_PASSWORD_MAX_LEN',15); //Security to prevent shell overflow.
	
    class FPDM {
    //@@@@@@@@@
		var $FPDM_FILTERS=array('ASCIIHexDecode', 'ASCII85Decode', 'FlateDecode', 'LZWDecode', 'Standard'); //holds all supported filters
		...
		**/
		...
		//----------------------
			...
			$info["Modes"]=array(
				...
				'compress'=>($this->compress_mode) ? 'Yes': 'No',
				'uncompress'=>($this->uncompress_mode) ? 'Yes': 'No',
		...
		**/
		...
		//-----------------------------------
			...
			if($this->support == "pdftk") {
				...
				$tmp_file=false;
				$pdf_file=resolve_path(fix_path(dirname(__FILE__).'../../../../web/'.$this->pdf_source));      //string: full pathname to the input pdf , a form file
				
				if($this->fdf_source) { //FDF file provided
					$fdf_file=resolve_path(fix_path(dirname(__FILE__).'../../../../web/'.$this->fdf_source));
				}else {
